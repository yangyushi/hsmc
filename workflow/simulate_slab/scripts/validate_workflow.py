...
def validate_configuration() -> None:
    ...
    crystal = _require_value(conf, "Boundary", "crystal").strip().lower()
    if crystal not in ("fcc", "hcp"):
        raise ValueError("[Boundary] crystal must be 'fcc' or 'hcp'.")
    hkl_raw = _require_value(conf, "Boundary", "hkl").split()
    if len(hkl_raw) != 3:
        raise ValueError("[Boundary] hkl must be three integers separated by spaces.")
    try:
        hkl = tuple(int(x) for x in hkl_raw)
    except ValueError as exc:
        raise ValueError("[Boundary] hkl must be three integers.") from exc
    if all(x == 0 for x in hkl):
        raise ValueError("[Boundary] hkl cannot be (0, 0, 0).")
    _require_float(conf, "Boundary", "vf_crystal", 0.0)
    _require_float(conf, "Boundary", "slab_thickness", 0.0)
    _require_float(conf, "Boundary", "z_aspect", 0.0)
    _require_float(conf, "Boundary", "min_gap", 0.0)
    _require_float(conf, "Boundary", "boundary_thickness", 0.0)

    nbins = _require_int(conf, "Analyse", "nbins", 0)
    if nbins == 0:
        pass  # auto-scale later
...
def validate_environment(
    ...
    # The crystal slab generator is vendored inside the workflow as
    # ``common.crystal_slab``.  The workflow runs with the workflow directory on
    # ``sys.path``, so this direct import is fine here.
    try:
        importlib.import_module("common.crystal_slab")
    except Exception as exc:
        missing_modules.append(("common.crystal_slab", str(exc)))
